...
import { Plus, Zap, LayoutDashboard, Eye } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const lineData = { labels: Object.keys(byHour).sort(), values: Object.keys(byHour).sort().map(k => byHour[k]) };
  const users = new Set(events.map(e => e.user_id).filter(Boolean)).size;

  const sendTest = () => ingest.mutate({ event_type: "page_view", event_name: "dashboard_visited", properties: { page: "/dashboard" }, user_id: "demo_user" });
  const handleCreate = async () => { if (!name.trim()) return; await create.mutateAsync({ name, description: desc }); setName(""); setDesc(""); setShowForm(false); };
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <Card><CardHeader><CardTitle>Events Over Time</CardTitle></CardHeader><CardContent>
          {lineData.labels.length > 0 ? <LineWidget data={lineData}/> : <div className="flex flex-col items-center justify-center h-56 text-gray-400"><Zap size={32} className="mb-2 opacity-30"/><p className="text-sm">{canIngest ? "Click Send test event above" : "No data yet"}</p></div>}
        </CardContent></Card>
        <Card><CardHeader><CardTitle>Event Types</CardTitle></CardHeader><CardContent>
          {pieData.labels.length > 0 ? <PieWidget data={pieData}/> : <div className="flex items-center justify-center h-56 text-gray-400"><p className="text-sm">No data yet</p></div>}
        </CardContent></Card>
      </div>

      <Card><CardHeader><CardTitle className="flex items-center gap-2"><LayoutDashboard size={18}/>Your Dashboards</CardTitle></CardHeader><CardContent>
        {dashboards.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {dashboards.map(d => (
              <div key={d.id} className="flex items-center justify-between py-4">
                <div><p className="font-medium text-sm text-gray-900">{d.name}</p><p className="text-xs text-gray-500">{d.description || "No description"} · {d.widget_count} widgets</p></div>
